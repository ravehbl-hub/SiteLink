/**
 * Camera / image capture (FR-FOR parity with Manager): the App supports camera
 * capture for the worker profile image + gallery pick. Wraps expo-image-picker so
 * screens get a simple {uri, fileName, mimeType, sizeBytes} back for the signed-URL
 * upload flow. Mirrors Frontend/manager/app/src/lib/camera.ts verbatim.
 */
...
/** Pick from the media library (gallery). Returns null if cancelled. */
