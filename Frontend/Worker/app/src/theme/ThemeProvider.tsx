...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // applyDirection flips the native flag and returns true — we must RELOAD ONCE
      // now so the app comes up in the correct direction with NO user round-trip.
      // After the reload isRTL matches shouldRtl, applyDirection returns false, and
      // we fall through to setReady — so this reloads at most once (no loop).
