 ...
 * - `rootGet` targets the ROOT origin (outside /api/v1) for the unauthenticated
 *   health probes consumed by the Back Office Dashboard status chips.
 */
import { getSupabase } from '../supabase/client';
import { env, isApiConfigured, apiOrigin } from '../env';
...
/**
 * GET against the back-end ROOT origin (outside /api/v1). Used only for the
 * root-mounted health probes. Returns both the parsed body and the HTTP status
 * so callers can distinguish 200 from the 503 that /health/db returns when the
 * database is down (the body still carries {status:'degraded',db:'down'}).
 */
export async function rootGet<T>(
  ...
  if (!origin) throw new ApiNotConfiguredError();
